...
paths = paths.map(path => path.map(coor => ({x: coor.x, y: coor.y})))
let map = Array.from({length: highest_y+1}, () => Array.from({length: highest_x+1}, () => 0))

map.push(Array.from({length: highest_x+1}, () => 0))
...
function calcMoveTo(pos) {
    if(pos.y === highest_y+1) return pos
...
while(true) {
    ...
    while(true) {
        ...
        if(next_pos.x === spawn_x && next_pos.y === 0) {
            sand_count++
            break main_loop;
        }
